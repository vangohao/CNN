...
localparam
    ...
    ADDR_IN_R_CTRL          = 6'h14,
    ADDR_OUT_R_DATA_0       = 6'h18,
    ADDR_OUT_R_CTRL         = 6'h1c,
    ADDR_W_DATA_0           = 6'h20,
    ADDR_W_CTRL             = 6'h24,
    ADDR_PARAMETER_R_DATA_0 = 6'h28,
    ADDR_PARAMETER_R_CTRL   = 6'h2c,
    WRIDLE                  = 2'd0,
    WRDATA                  = 2'd1,
    WRRESP                  = 2'd2,
    WRRESET                 = 2'd3,
    ...
    if (ACLK_EN) begin
        if (ar_hs) begin
            ...
            case (raddr)
                ...
                ADDR_PARAMETER_R_DATA_0: begin
                    rdata <= int_Parameter_r[31:0];
                end
            endcase
        end
    end
...
assign W           = int_W;
assign Parameter_r = int_Parameter_r;
// int_ap_start
always @(posedge ACLK) begin
    if (ARESET)
        int_ap_start <= 1'b0;
...
// int_Parameter_r[31:0]
always @(posedge ACLK) begin
    if (ARESET)
        int_Parameter_r[31:0] <= 0;
    else if (ACLK_EN) begin
        if (w_hs && waddr == ADDR_PARAMETER_R_DATA_0)
            int_Parameter_r[31:0] <= (WDATA[31:0] & wmask) | (int_Parameter_r[31:0] & ~wmask);
    end
end
