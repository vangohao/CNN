...
architecture behave of cnn_AXILiteS_s_axi is
    ...
    constant ADDR_IN_R_CTRL          : INTEGER := 16#14#;
    constant ADDR_OUT_R_DATA_0       : INTEGER := 16#18#;
    constant ADDR_OUT_R_CTRL         : INTEGER := 16#1c#;
    constant ADDR_W_DATA_0           : INTEGER := 16#20#;
    constant ADDR_W_CTRL             : INTEGER := 16#24#;
    constant ADDR_PARAMETER_R_DATA_0 : INTEGER := 16#28#;
    constant ADDR_PARAMETER_R_CTRL   : INTEGER := 16#2c#;
    ...
    signal int_Parameter_r     : UNSIGNED(31 downto 0) := (others => '0');
        ...
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (ar_hs = '1') then
                    ...
                    when ADDR_PARAMETER_R_DATA_0 =>
                        rdata_data <= RESIZE(int_Parameter_r(31 downto 0), 32);
                    when others =>
                        rdata_data <= (others => '0');
                    end case;
                end if;
...
-- ----------------------- Register logic ----------------
    ...
    Parameter_r          <= STD_LOGIC_VECTOR(int_Parameter_r);
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (w_hs = '1' and waddr = ADDR_PARAMETER_R_DATA_0) then
                    int_Parameter_r(31 downto 0) <= (UNSIGNED(WDATA(31 downto 0)) and wmask(31 downto 0)) or ((not wmask(31 downto 0)) and int_Parameter_r(31 downto 0));
                end if;
            end if;
        end if;
    end process;
