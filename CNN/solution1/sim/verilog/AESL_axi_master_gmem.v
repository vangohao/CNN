...
module AESL_axi_master_gmem (
    ...
    TRAN_gmem_Out_r,
    TRAN_gmem_W,
    TRAN_gmem_Parameter_r,
    ready,
    done
    );

//------------------------Parameter----------------------
`define TV_IN_gmem "../tv/cdatafile/c.cnn.autotvin_gmem.dat"
`define TV_OUT_gmem "../tv/rtldatafile/rtl.cnn.autotvout_gmem.dat"
...
parameter gmem_mem_depth = 32'd 18160;
...
output [32 - 1 : 0] TRAN_gmem_In_r;
output [32 - 1 : 0] TRAN_gmem_Out_r;
output [32 - 1 : 0] TRAN_gmem_W;
output [32 - 1 : 0] TRAN_gmem_Parameter_r;
input ready;
input done;
...
reg [gmem_DATA_BITWIDTH - 1 : 0] gmem_mem_3 [0: gmem_mem_depth - 1]; 
...
assign    TRAN_gmem_In_r = In_r;
assign    TRAN_gmem_Out_r = Out_r;
assign    TRAN_gmem_W = W;
assign    TRAN_gmem_Parameter_r = Parameter_r;
...
initial begin : initialize_offset
  ...
  In_r <= 0 * DATA_byte_num;
  Out_r <= 3072 * DATA_byte_num;
  W <= 17472 * DATA_byte_num;
  Parameter_r <= 17904 * DATA_byte_num;
end
    ...
    while (1) begin
        if (FIFO_AW_req_empty === 1 || BRESP_counter == 32) begin
            ...
            if (FIFO_AW_req_ADDR_tmp/data_byte_size > gmem_mem_depth) begin
                $display ("C:/Users/hao/Dropbox/txsx/CNN/CNN/solution1/sim/verilog/AESL_axi_master_gmem.v: Write request address %d exceed AXI master gmem array depth: %d",FIFO_AW_req_ADDR_tmp/data_byte_size, gmem_mem_depth); 
                $finish;
            end
    ...
    while (1) begin
        ...
        if (FIFO_AR_req_empty) begin
            ...
            if (FIFO_AR_req_ADDR_tmp/data_byte_size > gmem_mem_depth) begin
                $display ("C:/Users/hao/Dropbox/txsx/CNN/CNN/solution1/sim/verilog/AESL_axi_master_gmem.v: Read request address %d exceed AXI master gmem array depth: %d",FIFO_AR_req_ADDR_tmp/data_byte_size, gmem_mem_depth); 
                $finish;
            end
...
initial begin : read_file_process 
  ...
  while (token != "[[[/runtime]]]") begin 
      ...
      for(i = 3072; i < 17472 ; i = i + 1) begin 
          token = read_token(fp);
          ret = $sscanf(token, "0x%x", token_tmp); 
          if (factor == 4) begin
              if (i%factor == 0) begin
      ...
      for(i = 17472; i < 17904 ; i = i + 1) begin 
          token = read_token(fp);
          ret = $sscanf(token, "0x%x", token_tmp); 
          if (factor == 4) begin
              if (i%factor == 0) begin
      ...
      for(i = 17904; i < 18160 ; i = i + 1) begin 
          token = read_token(fp);
          ret = $sscanf(token, "0x%x", token_tmp); 
          if (factor == 4) begin
              if (i%factor == 0) begin
      ...
      if (factor == 2) begin
          if ((gmem_mem_depth - 1) % factor == 0) begin
              case(mem_page)
                  ...
                  3 : $fdisplay(fp,"0x%x",gmem_mem_3[gmem_mem_depth/factor][15:0]);
                  default: $display("The page_num of write file is not valid!");
              endcase
          end
      end
